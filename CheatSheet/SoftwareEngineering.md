...
12. [Тестирование программ и жизненный цикл программного продукта.](#тестирование-программ-и-жизненный-цикл-программного-продукта)
    1. [Тестирование в составе жизненного цикла:](#тестирование-в-составе-жизненного-цикла)
    2. [Тестирование примитивных программ](#тестирование-примитивных-программ)
...
Базисная модель вычисляет затраты разработки и ее стоимость как функцию размера программы. Промежуточная – учитывает атрибуты стоимости, включающие основные оценки продукта, аппаратуры, персонала, проектной среды. Усовершенствованная – объединяет все характеристики промежуточной модели, учитывая влияние всех атрибутов стоимости на каждый этап разработки ПО.
В моделях требуется информация о количестве строк в программе, количество объектных указателей, функциональных указателей. На выходе получаем затраты в человек-месяцах, время разработки в месяцах.
...
Наиболее распространенными _динамическим методом контроля_ является тестирование. **Тестирование** – это запуск программы на разных наборах исходных данных – тестах и анализ полученных результатов с целью обнаружения ошибок.

Искусство тестирования заключается в составлении набора тестов, минимальных по количеству, но обеспечивающих максимально возможную вероятность обнаружения ошибок. Необходимо проверить правильность программы на _всех возможных наборах исходных данных_. Это называется **полное тестирование**.

Начальное тестирование проводит автор программы, но для обеспечения должного уровня проверки **заключительное** тестирование должен проводить **НЕ автор**. Это имеет как объективные, так и субъективные причины. Объективная причина заключается в том, что программист часто не видит недостатки в своей же программе. Субъективный фактор заключается в том, что обнаружить ошибку в программе – значит признаться в своей ошибке. Кроме того, может случиться так, что сроки поджимают, работу надо сдать, но ещё не все в порядке.

[Еще : Тестирование и отладка.](#тестирование-и-отладка)


#### Тестирование в составе жизненного цикла:
В классическом жизненном цикле тестирование проводится после завершения разработки. При XP-программировании рекомендуется разработать тесты до написания кода. Очень хорошей можно считать практику, при которой на этапах анализа думают и о тестах.
...
В диаграмме необходимо отразить разные типы узлов сети и связи между ними. При проектировании программного обеспечения необходимо учитывать и многие особенности функционирования сетей. Например, можем ли мы рассчитывать, что компьютеры связаны между собой всегда, или должны предусмотреть и их автономную работу.
...
**Унифицированный процесс** - распространенная методология разработки по объектно-ориентированной методике. Позволяет команде разработки преобразовывать требования заказчика в работоспособный продукт. В зависимости от требований и доступных ресурсов, процесс разработки может быть адаптирован путём включения или исключения определённых проектных активностей.
...
Процесс изменения внутренней структуры программы, не затрагивающий её внешнего поведения и имеющий целью облегчить понимание её работы. Уровни рефакторинга (см еще ниже):
...
- Облегчение понимания программного комплекса в целом. В ходе разработки придется внести различные изменения и усовершенствования, в результате которых может теряться стройность системы в целом. Рефакторинг позволяет исправить ситуацию.
- Рефакторинг позволяет делать текст программы более понятным.
- Рефакторинг способствует нахождению ошибок в результате повышения качества кода.

Для успешного рефакторинга необходимо:
- Применять автоматизированное тестирование для проверки программ после каждого изменения.
- Сохранить текст программы без изменений в первоначальном виде, в случае неудачи можно к нему вернуться.
- Применять итерационную разработку (см. выше).
- Регулярно проводить рефакторинг.

Уровни:
•	“Маленький” рефакторинг. Не затрагивает структуру программы в целом.
•	“Большой” рефакторинг.
...
Средства рефакторинга в среде `Microsoft Visual Studio`.

#### "Малый" рефакторинг:
- **переименование**
  Можно переименовать все введенные программистом имена, независимо от того, что они обозначают.
...
- **удаление параметра**
  Выделим список параметров функции в самой функции или в любом месте ее вызова, откроем описанным выше способом диалоговое окно, где будут показаны все параметры. Выберем из них нужный и удалим его. Удаление будет автоматически распространено.
- **инкапсуляция поля**
  Под этим термином понимается создание свойства, представляющее переменную и/или массив
- **создание интерфейса**
  Для создания интерфейса выделим компонент класса, активизируем контекстное меню, дойдем до пункта контекстного меню «Создать интерфейс». Откроется диалоговое окно, в котором мы должны выбрать включаемые в создаваемый интерфейс компоненты класса и выбрать ему имя, если не устраивает имя по умолчанию. Будет создан интерфейс и класс, из которого он создавался, станет его наследником.
- **извлечение метода**
  Можно выделить часть кода (например, цикл) и выбрать “Извлечение функции” – получится новый метод.
...
Если интерфейс в Java содержит один и только один абстрактный метод, то он называется функциональным. Этот единственный метод определяет назначение интерфейса. Используется аннотация  `@FunctionalInterface`, которая помогает понять компилятору, что интерфейс функциональный.


Наиболее важные функциональные интерфейсы в Java:
...
Все классы, которые начинаются с java. или javax. лежат в JDK, а значит, являются стандартными.
...
// для разбивки форматированного ввода на слова и преобразования их в соответствии с их типом
import java.util.Scanner;
Scanner s = new Scanner(new BufferedReader(new FileReader("xanadu.txt")));
// ИЛИ : Scanner s = new Scanner(System.in); // стандартный ввод
double sum = 0;
while (s.hasNext()) {
  if (s.hasNextDouble()) {
    sum += s.nextDouble();
...
![Классы Пакета JavaUtil](КлассыПакетаJavaUtil.png)
